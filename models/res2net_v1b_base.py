# Derived from the Res2Net-PretrainedModels implementation:
...
def Res2Net_model(ind=50):
    ...
    # incompat = model.load_state_dict(model_dict, strict=False)
    # matched = sum(1 for k, v in pretrained_dict.items()if k in model_dict and v.shape == model_dict[k].shape)
    # print("=> Loaded ImageNet pretrained into Res2Net_Ours:",
    #     f"copied_from_ckpt={matched}/{len(pretrained_dict)}",
